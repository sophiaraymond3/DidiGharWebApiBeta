...
namespace DidiGharWebApi.Controllers
{
    ...
    public class RequestController : ApiController
    {
        ...
        [Route("GetTimeSlots")]
        public List<TimeSlotResponse> GetTimeSlots(int serviceId, int cityId, DateTime date)
        {
            DateTime startOfTheDay = DateTime.Now.StartOfDay(date);
            DateTime endOfTheDay = DateTime.Now.EndOfDay(date);

            if (date.AddHours(1) >= endOfTheDay)
            {
            ...
            else
            {
                ...
                for (int i = 0; i <= 18; i++)
                {
                    timeSlots.Add(new TimeSlotResponse()
                    {
                        ...
                        Disabled = (date.TimeOfDay >= intervalTimeSlot.TimeOfDay)?false:true
                    });
                    intervalTimeSlot = intervalTimeSlot.AddMinutes(30);
                }

                List<UpdatedTimeSlots> UpdatedTime = new List<UpdatedTimeSlots>();
                var totalRequests = db.Requests.Where(x => x.Status != 0 && DbFunctions.TruncateTime( x.RequestedOnUtc) == date.Date).ToList();
                if (totalRequests.Count > 0)
                {
                    ...
                    if (!partnersAvailable)
                    {
                        ...
                        foreach (var appointmemt in UpdatedTime)
                        {
                            ...
                            if (!slotFree)
                            {
                                timeSlots.Where(x => x.TimeSlot.TimeOfDay >= appointmemt.StartTimeSlot.TimeOfDay && x.TimeSlot.TimeOfDay <= appointmemt.EndTimeSlot.TimeOfDay).ToList().ForEach(f => f.Disabled = false);
                            }
                        }
                    }
                }
